...
be triggered by an end admin-user (such as which items are to be selected). To a large
...
be available via the API - so the end admin-user wouldn't be able to select rows
...
```
{
	items:string     - Can be `rows`, `columns` or `cells`. Defines what item 
	                   will be selected if the admin-user is allowed to activate row
...
// DataTables will rework the events and admin-address this.
